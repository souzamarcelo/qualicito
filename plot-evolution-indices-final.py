...
for i in range(len(indicadores)):
    ...
    for estado in estados:
        col = '#303030'
        ax.plot(d[d['estado'] == estado]['ano'], d[d['estado'] == estado][indicadores[i]], label = legendas[estado] , marker = marcadores[c], linestyle = '-', markersize = 4.3, color = col, markeredgecolor = col, markerfacecolor = 'white', linewidth = .7, markeredgewidth = .5)
        c += 1
    for value in baselines[i]:
        lim = ax.get_ylim()
        if value > lim[0] and value < lim[1]:
            ax.axhline(y = value, linewidth = 1, color = 'black', linestyle = '--')
    ax.set_xticks(anos)
    #ax.legend()
    ax.grid(False)
...
plt.savefig('plot-evolution-indices-final.eps', format = 'eps', dpi = 1200)
plt.savefig('plot-evolution-indices-final.pdf', format = 'pdf', dpi = 1200)